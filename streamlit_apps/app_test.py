...
with st.sidebar:
    ...
    top_n = st.slider("Select number of top entities", min_value=1, max_value=50, value=10)

# Trigger data download and processing
if st.button('Download and Process Data'):
    st.write(f"Downloading GDELT GkG 1.0 files from {start_date} to {end_date}")
    download_and_process_data(start_date, end_date)  # this would be a function based on your script
    st.success("Data downloaded and processed.")

# Display a map visualization
if st.button('Show Location Map'):
    df = pd.read_json('gkg1.0_7day.json')
    location_counter = filter_and_count_locations(df)
    fig, ax = plt.subplots()  # Create a figure for the plot
    plot_minimalist_map(location_counter, 'Country heatmap', color_map='Greens', power=0.5, ax=ax)
    st.pyplot(fig)

# Display bar charts for themes and organizations
if st.button('Show Visualizations'):
    df = pd.read_json('gkg1.0_7day.json')
    generate_visualizations(df, top_n)
    # The `generate_visualizations` function needs to be adapted to work with Streamlit and return figures.

# Timeline Heatmap
if st.button('Show Timeline Heatmap'):
    df = pd.read_json('gkg1.0_7day.json')
    theme_counts_df, theme_tone_df = prepare_heatmap_data(df, top_n)
    fig = plot_side_by_side_heatmaps(theme_counts_df, theme_tone_df, top_n)
    st.pyplot(fig)
